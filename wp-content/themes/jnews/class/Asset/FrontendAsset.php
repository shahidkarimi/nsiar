...
Class FrontendAsset extends AssetAbstract {
	...
	public function __construct() {
		...
		if ( $this->is_debugging && ! is_user_logged_in() ) {
			...
			add_action( 'jeg_before_inline_dynamic_css', [ $this, 'start_inline_dynamic_css' ], 1 );
			add_action( 'jeg_after_inline_dynamic_css', [ $this, 'end_inline_dynamic_css' ], 99 );
			add_action( 'wp_footer', [ $this, 'render_inline_dynamic_css' ], 99 );
		}
	...
	public function load_jnews_library() {
		$library = $this->load_file( get_parent_theme_file_path( 'assets/js/jnewslibrary.js' ) );
	...
	public function preload_style( $html, $handle ) {
		...
		if ( in_array( $handle, $preload_style, true ) ) {
			...
			$version = in_array( $handle, [ 'elementor-font-awesome-webfont', 'font-awesome-webfont' ], true ) ? '?v=' : '?ver=';
			$html    = str_replace(
	...
	public function autoptimize_store_js( $url ) {
		if ( strpos( $url, '.js' ) > 0 ) {
			...
			$jnews_au_scripts = is_array( $jnews_au_scripts ) ? $jnews_au_scripts : [];
	...
	public function autoptimize_script_loader() {
		if ( function_exists( 'autoptimize' ) ) {
			if ( autoptimize()->should_buffer() ) {
				...
				ob_start( [ '\JNews\\' . jnews_custom_text( 'xajAdnetnorF' ), jnews_custom_text( 'gnireffub_dne' ) ] );
			}
		}
	}
	...
	public function load_style() {
		...
		if ( ! $this->is_login_page() ) {
            ...
            wp_register_style( 'jnews-global-slider', $asset_url . 'css/slider/jnewsglobalslider.css', [ 'tiny-slider' ], $theme_version );
			wp_register_style( 'jnews-videoplaylist', $asset_url . 'css/jnewsvidplaylist.css', [ 'jnews-global-slider' ], $theme_version );
			wp_register_style( 'jnews-newsticker', $asset_url . 'css/jnewsticker.css', null, $theme_version );
			wp_register_style( 'jnews-hero', $asset_url . 'css/jnewshero.css', [ 'jnews-global-slider' ], $theme_version );
			wp_register_style( 'jnews-overlayslider', $asset_url . 'css/joverlayslider.css', [ 'jnews-global-slider' ], $theme_version );
			
			if ( $this->is_debugging ) {
				/***** Code harus di sync dengan webpack config ****/
				wp_register_style( 'jnews-slider', $asset_url . 'css/slider/jnewsslider.css', [ 'jnews-global-slider' ], $theme_version );
				wp_register_style( 'jnews-carousel', $asset_url . 'css/slider/jnewscarousel.css', [ 'jnews-global-slider' ], $theme_version, false );
